...
int main(int argc, char *argv[]) {
	...
	if(result12 < 0) { // если не удалось создать ппроцесс 2
		...
		int    fd23, result23; // для передачи от процесса 2 к процессу 3
		char    name23[] = "bbb.fifo";
		mknod(name23, S_IFIFO | 0666, 0);
		// пытаемся создать процесс 3
		result23 = fork();
		if(result23 < 0) { // если не удалось создать процесс 3
	 		printf("Can\'t fork child\n");
	 		exit(-1);
		} else if (result23 > 0) { /*процесс 2*/
			// процесс 2 пишет в bbb.fifo
			if ((fd23 = open(name23, O_WRONLY, 0666)) < 0) {
				printf("coudlnt open the FIFO");
				exit(-1);
			}
			write(fd23, buffer2, MAX_BUFFER_SIZE);
			if (close(fd23) < 0) {
				printf("coudlnt close the FIFO");
				exit(-1);
			}
			...
			// процесс 3 читеат из bbb.fifo
			if((fd23 = open(name23, O_RDONLY)) < 0) {
				printf("Can\'t open FIFO for reading\n");
				exit(-1);
			}
			read(fd23, buffer3, MAX_BUFFER_SIZE);
			if(close(fd23) < 0) {
				printf("child: Can\'t close FIFO\n"); exit(-1);
			}
